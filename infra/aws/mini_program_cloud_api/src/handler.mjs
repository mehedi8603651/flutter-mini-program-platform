...
export const handler = async (event) => {
  ...
  try {
    ...
    if (
      ...
      pathSegments[0] === 'api' &&
      pathSegments[1] === 'discovery' &&
      isCatalogSegment(pathSegments[2])
    ) {
    ...
    if (
      ...
      pathSegments[0] === 'api' &&
      pathSegments[1] === 'manifests' &&
      isLatestSegment(pathSegments[3])
    ) {
      return handleLatestManifest({
        traceId,
    ...
    if (
      ...
      pathSegments[0] === 'api' &&
      pathSegments[1] === 'debug' &&
      pathSegments[2] === 'manifests' &&
      isDecisionSegment(pathSegments[4])
    ) {
      return handleDebugDecision({
        traceId,
    ...
    if (
      ...
      return handleVersionedManifest({
        traceId,
        miniProgramId: pathSegments[2],
        version,
        event,
...
async function handleLatestManifest({ traceId, miniProgramId, query, event }) {
  ...
  const extraHeaders = {
    ...
    ...(decision.matchedRuleId
      ? { 'x-mini-program-matched-rule-id': decision.matchedRuleId }
      : {}),
  };
...
async function jsonFromS3Object({ traceId, key, notFoundMessage, extraHeaders = {} }) {
  ...
  const decodedJson = parseJsonValue(rawJson, key);

  return jsonResponse({
    statusCode: 200,
    bodyObject: decodedJson,
    traceId,
    extraHeaders,
  });
}
...
async function resolveManifestDecision({ miniProgramId, query }) {
  ...
  if (pinnedVersion) {
    ...
    await assertObjectExists(
      manifestKey,
      `Pinned version "${pinnedVersion}" for mini-program "${miniProgramId}" was not found.`,
    );
    return {
...
function resolveTraceId(event) {
  const requestedTraceId = nullIfBlank(
