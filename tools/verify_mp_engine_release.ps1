...
Write-Host "- Static artifacts: miniprogram publish --target static --mini-program-root <fixture> --output <public_static_folder> --clean --json"
Write-Host "- Publisher API: run contract validate/smoke against a disposable middle server."
Write-Host "- Host import: import a handoff package into a disposable host and run flutter run -d chrome."
Write-Host "- Do not run destroy, delete, cleanup, or other destructive external commands from this script."
Write-Host "- Full commands: docs/static_artifact_runtime_api_e2e_guide.md"
