...
class _MiniProgramListPageState extends State<MiniProgramListPage> {
  ...
  Future<List<PartnerMiniProgramDefinition>>? _remoteProgramsFuture;
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: const Text('Partner App Host'),
        actions: [
          IconButton(
  ...
  List<Widget> _buildProgramSection() {
    ...
    if (remoteCatalogClient == null) {
      return _buildProgramCards(PartnerMiniProgramCatalog.availablePrograms);
    }

    return <Widget>[
      FutureBuilder<List<PartnerMiniProgramDefinition>>(
        ...
        builder: (context, snapshot) {
          ...
          if (snapshot.hasError) {
            return Column(
              ...
              children: [
                _CatalogNotice(
                  message:
                      'Remote catalog discovery failed. Falling back to the local partner list.',
          ...
          final programs =
              snapshot.data ?? const <PartnerMiniProgramDefinition>[];
          if (programs.isEmpty) {
  ...
  List<Widget> _buildProgramCards(List<PartnerMiniProgramDefinition> programs) {
    return programs
        .map(
          (program) => Padding(
  ...
  void _refreshProgramState() {
    ...
    _registerDiscoveryFutures(PartnerMiniProgramCatalog.availablePrograms);
  ...
  Future<List<PartnerMiniProgramDefinition>> _loadRemotePrograms(
    ...
    final programs = catalog.entries
        .map(PartnerMiniProgramDefinition.fromPublishedSummary)
        .toList(growable: false);
  ...
  void _registerDiscoveryFutures(
    Iterable<PartnerMiniProgramDefinition> programs,
  ) {
          ...
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                const SizedBox(height: 12),
                Text(
                  program.isBackendDiscovered
                      ? 'Resolved lane: ${program.expectedLaneVersion}'
