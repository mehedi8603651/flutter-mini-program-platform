...
void main() {
  group('ManagedStacBuilder', () {
    ...
    test('copies the bundled template into a user cache and bootstraps it', () async {
      ...
      await File(p.join(templateRoot, 'pubspec.yaml')).writeAsString('name: stac_cli');
      await File(
      ...
      expect(
        await File(
          p.join(resolution.packageRootPath, '.dart_tool', 'package_config.json'),
        ).exists(),
        isTrue,
      );
