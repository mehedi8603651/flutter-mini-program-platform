...
void main() {
  ...
  group('MiniProgramPreviewController', () {
    ...
    test(
      ...
      () async {
        ...
        expect(
          processCall!.arguments.any(
            (argument) => argument.startsWith(
              '--dart-define=MINI_PROGRAM_PREVIEW_BACKEND_BASE_URL=',
            ),
          ),
          isFalse,
        );
        expect(stdoutBuffer.toString(), isNot(contains('Publisher API:')));
        expect(
          stdoutBuffer.toString(),
          isNot(contains('Runtime middle-server API:')),
        );
    ...
    test(
      'passes optional runtime middle-server API base URL to the host app',
      () async {
        final fixture = await _writePreviewBuildFixture(
          tempDir.path,
          miniProgramId: 'coupon_center',
        ...
        expect(
          stdoutBuffer.toString(),
          contains('Runtime middle-server API: http://127.0.0.1:9090/'),
        );
      },
    );
