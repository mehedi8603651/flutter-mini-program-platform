...
- ignores generated `mp/.build` output
...
repo should not contain development folders like `.dart_tool/`, `mp/.build/`,
...
- Seed data lives in
  ...
  `backend/firebase_functions/functions/data/session.json`. These JSON files
...
Do not edit `mp/.build` directly; it is generated by build/publish. After UI
edits, run `miniprogram build` or publish again. After
...
Use `Mp.backend.call(...)` when you need a backend write or command action
without storing state for `{{backend.*}}` bindings.
...
- a README button snippet showing how to open a registered mini-program

Import a partner handoff package or add a public Mp delivery endpoint after
the host wiring exists.
