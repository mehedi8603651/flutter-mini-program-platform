...
|       |   |-- validation.dart                  # IDs, capabilities, formats, titles, and descriptions
|       |   |-- paths.dart                       # Repository/standalone target resolution and overwrite checks
|       |   |-- dependencies.dart                # Local package discovery and generated pubspec dependency text
|       |   |-- managed_files.dart               # Stable managed-file assembly, write order, and mock adapter
|       |   |-- coordinator.dart                 # Ordered scaffold validation and generation orchestration
|       |   `-- templates/
...
Mini-program scaffold public request, result, error, and coordinator types stay
available through `mini_program_scaffolder.dart`. Implementation belongs in
normal Dart libraries under `scaffolding/` and must not import the public
tooling barrel or scaffolder facade. Preserve validation order and exact error
text, repository and standalone path behavior, capability ordering, local
dependency resolution, managed-file insertion and write order, generated bytes,
force-overwrite behavior, unrelated files, and mock Publisher API append order.
